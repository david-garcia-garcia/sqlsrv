   ...
   */
    ...
    public function initializeDatabase()
    {
      ...
      /** @var \Drupal\Driver\Database\sqlsrv\Connection $database */
            $connection = Database::getConnection();
            Utils::DeployCustomFunctions($connection->getConnection(), dirname(__FILE__) . '/../Programability');
            $this->pass(t('SQLServer has initialized itself.'));
            // Ensure we clear the schema cache
            $connection->getConnection()->Cache('sqlsrv_meta')->Clear(null);
            $connection->getConnection()->Cache('sqlsrv-table-exists')->Clear(null);
            $connection->getConnection()->Cache('sqlsrv-tabledetails')->Clear(null);
        } catch (\Exception $e) {

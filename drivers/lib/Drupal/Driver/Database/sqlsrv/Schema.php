...
class Schema extends DatabaseSchema
{
  ...
  public function createTable($name, $table)
  {
    ...
    if (!empty($table['description'])) {
      if ($this->tableExists($name)) {
        ...
        $this->CommentCreateOrUpdate($comment, $name);
      }
    }
    // Add column comments.
    foreach ($table['fields'] as $field_name => $field) {
      if (!empty($field['description'])) {
        $comment = $this->prepareComment($table['description'], Scheme::COMMENT_MAX_BYTES);
        $this->CommentCreateOrUpdate($comment, $name, $field_name);
      }
    }
  ...
  public function CommentGet($table, $column = null)
  {
    ...
    $extended_property = $this->connection->GetConnection()->query_execute($args['query'], $args['args'])->fetchAssoc();
   ...
   *
   * @param $value
   *   The actual comment, use null or empty string to remove the comment
   * @param null $table
   *   The table
   * @param null $column
   *   The column
  ...
  public function CommentCreateOrUpdate($value, $table = null, $column = null): void
  {
    ...
    if (isset($columns[$key]) && $this->connection->GetConnection()->inTransaction()) {
      $result = $columns[$key];
    } else {
  ...
  public function addField($table, $field, $spec, $new_keys = [])
  {
    ...
    if (!empty($spec['description'])) {
      ...
      $this->CommentCreateOrUpdate($comment, $table, $field);
    }
    // Clear column information for table.
    $this->getTableIntrospectionInvalidate($table);
  }
  ...
  {
    return $this->CommentGet($this->connection->prefixTables("{{$table}}"), $column);
  }
